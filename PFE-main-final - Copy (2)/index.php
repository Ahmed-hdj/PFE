...
<body class="open-sans" style="font-family: 'Open Sans', sans-serif;">
    <div class="pt-24">
        ...
        <div class="relative h-[75vh] w-full before:absolute before:inset-0 before:bg-black/58 before:z-10">
            ...
            <video src="images and videos/Algeria from Above 4K UHD - A Cinematic Drone Journey (1).mp4" loop muted
                autoplay class="w-full h-full object-cover" playbackRate="1.5"></video>
        </div>
        ...
        <div id="addPlaceModal"
            class="fixed inset-0 bg-black/70 backdrop-blur-[1px] hidden items-center justify-center z-50 transition-opacity duration-200">
            <div
                class="bg-white p-8 rounded-lg w-[90%] max-w-[600px] max-h-[90vh] shadow-xl overflow-hidden flex flex-col border-[6px] border-[#327532]">
                <div class="flex justify-between items-center mb-4 bg-white sticky top-0 z-10 pb-4 border-b">
                    <img src="images and videos/ChatGPT Image 29 avr. 2025, 00_41_42.png" alt=""
                        class="w-[70px] mr-auto">
                    <h2 class="text-2xl font-bold text-[#19a719]">Ajouter un nouveau lieu</h2>
                    <button onclick="hideAddPlaceModal()"
                        class="text-gray-500 hover:text-gray-700 cursor-pointer ml-auto">
                        <i class="fas fa-times text-[18px]"></i>
                    </button>
                </div>
                <div class="overflow-y-auto flex-1">
                    <form id="addPlaceForm" class="space-y-6" method="POST" enctype="multipart/form-data">
                        <div>
                            <label class="block text-gray-700 font-medium mb-2">Nom du lieu :</label>
                            <input type="text" name="title"
                                class="w-full px-4 py-2.5 border border-[#a1a1a1] rounded-full focus:outline-none focus:shadow-[0_0_6px_1px_#23c523] transition-all duration-300"
                                placeholder="Entrez le nom du lieu" required>
                        </div>
                        <div>
                            <label class="block text-gray-700 font-medium mb-2">Description :</label>
                            <textarea name="description"
                                class="w-full px-4 py-2.5 border border-[#a1a1a1] rounded-lg focus:outline-none focus:shadow-[0_0_6px_1px_#23c523] transition-all duration-300 resize-none"
                        ...
                        <div>
                            <label class="block text-gray-700 font-medium mb-2">Wilaya :</label>
                            <select name="wilaya"
                                class="w-full px-4 py-2.5 border border-[#a1a1a1] rounded-full focus:outline-none focus:shadow-[0_0_6px_1px_#23c523] transition-all duration-300 appearance-none bg-white"
                                required>
                                ...
                                <div class="flex items-center gap-2">
                                    <label class="flex-1">
                                        <input type="file" name="images[]" class="hidden" multiple accept="image/*"
                                            id="addPlaceImageUpload" onchange="previewAddPlaceImages(this)">
                                        <div
                                            class="w-full px-4 py-2.5 border border-[#a1a1a1] rounded-full cursor-pointer hover:bg-gray-50 transition-all text-center group">
                                            <i
                                                class="fas fa-cloud-upload-alt mr-2 text-gray-500 group-hover:text-gray-700"></i>
                                            <span class="text-gray-600 group-hover:text-gray-800">Select Images</span>
                                        </div>
                                    </label>
                                    <button type="button"
                                        onclick="document.getElementById('addPlaceImageUpload').click()"
                                        class="px-4 py-2.5 bg-gray-100 rounded-full hover:bg-gray-200 transition-all text-gray-600 hover:text-gray-800">
                                        <i class="fas fa-plus"></i>
                                    </button>
                                </div>
                                <div id="addPlaceImagesPreview"
                                    class="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[200px] overflow-y-auto p-3 bg-gray-50 rounded-lg hidden">
                ...
                <div class="sticky bottom-0 bg-white pt-4 border-t mt-4">
                    <button type="submit" form="addPlaceForm"
                        class="w-full bg-[#343a40] text-white py-3 rounded-full hover:bg-[#495057] transition-all font-bold shadow-sm hover:shadow-md">
                        Ajouter le lieu
                    </button>
                </div>
            </div>
                    ...
                    <form id="modifyLieuForm" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="modifyTitle" name="title"
                                class="w-full px-3 py-2 border rounded-full" placeholder="Entrez le nom du lieu"
                                required>
                        </div>
                        <div>
                            <label class="block text-gray-700 mb-2">Description</label>
                            <textarea id="modifyContent" name="content" class="w-full px-3 py-2 border rounded-lg"
                                rows="3" placeholder="Entrez la description du lieu" required></textarea>
                        </div>
                        <div>
        ...
        <script>
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        result.data.forEach(lieu => {
                            console.log('Creating card for lieu:', lieu.lieu_id, 'Is Saved:', lieu.is_saved);
            ...
            // Update the createLieuCard function to match dynamic-page categories styling
            ...
            `).join('');
                ...
                // Determine if the place is saved by the current user
                const isSaved = lieu.is_saved > 0 ? 'fas' : 'far';
                ...
                <div class="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 h-full">
                    <div class="relative">
                        <div class="relative h-72 image-gallery" data-gallery="${lieu.lieu_id}">
                            ${imageGalleryHtml}
                            <!-- Navigation arrows -->
                            <button class="absolute top-1/2 left-2 transform -translate-y-1/2 bg-white p-1 rounded-full shadow opacity-75 hover:opacity-100 transition-opacity z-20 prev" onclick="event.preventDefault(); event.stopPropagation();"><i class="fas fa-chevron-left text-gray-800"></i></button>
                            <button class="absolute top-1/2 right-2 transform -translate-y-1/2 bg-white p-1 rounded-full shadow opacity-75 hover:opacity-100 transition-opacity z-20 next" onclick="event.preventDefault(); event.stopPropagation();"><i class="fas fa-chevron-right text-gray-800"></i></button>
                            <!-- Dots for pagination -->
                            ...
                            <div class="absolute top-4 left-4 z-30">
                                <button onclick="event.preventDefault(); showLieuOptions(${lieu.lieu_id})"
                                    class="bg-white p-2 rounded-full shadow-lg hover:bg-gray-100 transition-colors py-[4px] px[15px] hover:cursor-pointer">
                                ...
                                <div id="lieuOptions${lieu.lieu_id}" class="hidden absolute left-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-40">
                                    <button onclick="event.preventDefault(); showModifyLieuModal(${lieu.lieu_id})"
                                        class="w-full text-left px-4 py-2 hover:bg-gray-100 hover:cursor-pointer">
                                        <i class="fas fa-edit mr-2"></i> Modify
                                    </button>
                                    <button onclick="event.preventDefault(); deleteLieu(${lieu.lieu_id})" class="w-full text-left px-4 py-2 text-red-600 hover:bg-gray-100 hover:cursor-pointer">
                            ...
                            <button onclick="event.preventDefault(); toggleSave(${lieu.lieu_id}, this)"
                                ...
                                <i class="${isSaved} fa-bookmark text-gray-600"></i>
                            </button>
                        </div>
                    </div>
                    <div class="p-4">
                        <h2 class="text-xl font-semibold mb-2">${lieu.title}</h2>
                        <p class="text-gray-600 text-sm mb-2">${lieu.content.substring(0, 100)}${lieu.content.length > 100 ? '...' : ''}</p>
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            if (data.action === 'saved') {
                                ...
                                // Keep or add dark gray color for saved
                                icon.classList.add('text-gray-600');
                            } else {
                                // Remove the class and revert color when unsaved
                                icon.classList.remove('fas');
                                icon.classList.add('far');
                                // Ensure dark gray color for unsaved
                                icon.classList.add('text-gray-600');
                            }
                        } else {
            ...
            document.getElementById('addPlaceForm').addEventListener('submit', async function (e) {
                ...
                try {
                    ...
                    if (result.success) {
                        hideAddPlaceModal();
                        this.reset();
                        document.getElementById('addPlaceImagesPreview').innerHTML = '';
                        document.getElementById('addPlaceImagesPreview').classList.add('hidden');
